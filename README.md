...
The backend Dockerfile installs production workspace dependencies again in the runtime stage, which keeps Railway's runtime module resolution reliable for Express dependencies. Redis is optional; without `REDIS_URL`, the backend uses in-memory rate limiting and crash round state for the current instance.
