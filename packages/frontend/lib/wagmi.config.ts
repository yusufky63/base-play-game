...
import { BASE_MAINNET_RPC_URLS, BASE_SEPOLIA_RPC_URLS } from "@baseplay/shared/config/networks";
import { walletConnectProjectId } from "@/lib/env";

const baseSepoliaTransports = BASE_SEPOLIA_RPC_URLS.map((url) => http(url, { timeout: 8_000, retryCount: 1, retryDelay: 250 }));

const baseMainnetTransports = BASE_MAINNET_RPC_URLS.map((url) => http(url, { timeout: 8_000, retryCount: 1, retryDelay: 250 }));
