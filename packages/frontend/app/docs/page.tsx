...
const faqs = [
  ...
  },
  {
    ...
    answer: "BasePlay uses standard wagmi and viem wallet flows with a global explicit wallet picker for normal web sessions, including injected wallets, WalletConnect, Coinbase Wallet, and Base Account. The picker renders at app-provider level, so header, mobile menu, and game-page connect actions all open the same centered modal on web. Farcaster and Base App sessions skip the modal and connect through their native connector. Injected options are hidden when the browser has no injected provider, and WalletConnect is shipped as a direct frontend dependency instead of relying on wagmi optional dependency resolution. The canonical app URL is https://baseplay.games, Base mainnet is the default chain, game actions open the wallet picker first on web and then request Base mainnet switching when the wallet is on an unsupported network, wallet transactions include the BasePlay Builder Code attribution suffix, fallback contract status reads are batched with multicall to reduce RPC load, and the listing copy uses Play. Compete. Win on Base."
  },
  {
